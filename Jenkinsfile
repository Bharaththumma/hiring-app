            ...
            steps {
                script{
                    withCredentials([string(credentialsId: 'git_tocken', variable: 'GIT_TOKEN')]) { 
                        ...
                        sed -i "s/16/${BUILD_NUMBER}/g" /var/lib/jenkins/workspace/$JOB_NAME/dev/deployment.yaml
                        cat /var/lib/jenkins/workspace/$JOB_NAME/dev/deployment.yaml
                        git add .
                        git commit -m 'Updated the deploy yaml | Jenkins Pipeline'
                        git remote -v
                        git push https://$GIT_TOKEN@github.com/Bharaththumma/Hiring-app-argocd.git main
                        '''                        
                      }
                  }
            }   
